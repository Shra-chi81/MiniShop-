...
export default function AdminDashboard() {
  ...
  const handleEdit = (p) => {
    setEditingId(p._id);
    setForm({ name: p.name, description: p.description, price: p.price, category: p.category, brand: p.brand, stock: p.stock, images: p.images || [] });
    setTab('products');
  };
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-3 h-fit">
            ...
            <div>
              ...
              <input type="file" accept="image/png,image/jpeg,image/jpg,image/webp,image/gif" multiple
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={addUrlManually}
                className="px-3 py-2 rounded-md border dark:border-gray-600 text-gray-700 dark:text-gray-200">Add</button>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {form.images.map((url, idx) => (
                  <div key={idx} className="relative w-16 h-16">
                    ...
                    <button type="button" onClick={() => removeImage(idx)} aria-label="Remove image"
          ...
          <div className="space-y-3 max-h-[600px] overflow-y-auto">
            {products.map((p) => (
              <div key={p._id} className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow flex items-center gap-3">
                <img src={p.images?.[0] || 'https://placehold.co/60x60'}
                ...
                <button onClick={() => handleDeleteClick(p._id)} className="text-red-500 text-sm">Delete</button>
              </div>
            ))}
          </div>
        </div>
